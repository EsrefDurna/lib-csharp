...
using System.IO;
using System.Text;
using Concur.Infrastructure.Services.Demo;
...
namespace Concur.Babel.Test
{
    ...
    },
    {
      ...
      ""Vojta"": [
		...
		public class GetByIdRequest : Concur.Babel.Mvc.IBabelRequest
		{
			/// <summary>
  ...
  ""companyId"": 1
		...
		private void SpeedTest(object data, Action<object, BabelJsonSerializer> babelSerialization, Action<object, Newtonsoft.Json.JsonSerializer> nkSerialization)
		{
			...
			sw.Stop();
			long tConcur1 = sw.ElapsedMilliseconds;
			sw.Reset();
			sw.Start();
			for(int i = 0; i < NUM_RUN; i++)
			{
			...
			sw.Stop();
			long tConcur2 = sw.ElapsedMilliseconds;
			sw.Reset();
			sw.Start();
			for(int i = 0; i < NUM_RUN; i++)
			{
			...
			Assert.Fail(string.Format("BabelJSONSerializer: {0}\t\t {1}\r\nNewtonsoft: {2}\t\t {3}", ((double)tConcur1) / NUM_RUN, ((double)tConcur2) / NUM_RUN, ((double)tNK1) / NUM_RUN, ((double)tNK2) / NUM_RUN));
