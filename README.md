...
**App_Code Considerations**

If you are using App_Code in your Babel web service, you should generate models into your App_Code directory, even if you must also generate models into an external clients directory.  These are generated files, so it's ok to gen them to 2 destinations from one central .babel file.  This ensures that your service doesn't generate errors in the time between when you commit changes, and when the project gets rebuilt.  
...
*Note on use of MVC Contoller vs WebAPI*: it was decided to not spend any time on implementing WebAPI support due to the unnecessary problems with the routing including forcing HTTP verbs on methods with the names starting with any HTTP verb and polymorphic behavior.
