...
namespace Concur.Babel
{
	...
	public class HttpTransport : IBabelTransport
	{
		...
		/// <summary>
		/// Makes synchonous HTTP request. 500 and other HTTP errors will not cause exception to be thrown. Result status code and error flag sould be used to process these situations.
		/// </summary>
		...
		// httpmethod, url, SendData, ContentType, isAsync, headerDict
		// virtual for testing
		...
		/// <summary>
		/// Makes asynchonous HTTP request. 500 and other HTTP errors will not cause exception to be thrown. Result status code and error flag sould be used to process these situations.
		/// </summary>
		/// <param name="httpMethod">Http method (GET, POST, ...))</param>
		/// <param name="method">Method to call</param>
		/// <param name="requestData">Data to send</param>
		/// <param name="contentType">Content type ("text/xml; charset=utf-8")</param>
		/// <param name="acceptType">The value of the Accept HTTP header(null (default), "text/xml", ...)</param>
		/// <param name="timeoutInSeconds">HTTP timeout in seconds</param>
		/// <param name="headers">Optional request headers</param>
		/// <param name="hasResponse">Doed it has response to deserialize</param>
		/// <param name="retriesLeft">How many attempts have left</param>
		/// <returns>Deserialized response</returns>
		// httpmethod, url, SendData, ContentType, isAsync, headerDict
		/*private Task<T> MakeHTTPRequestAsync(HttpMethod httpMethod, string method, object requestData, string contentType = null, string acceptType = null, int? timeoutInSeconds = null, NameValueCollection headers = null, bool hasResponse = true, int retriesLeft = 0)
		{
			if (string.IsNullOrEmpty(method)) throw new ArgumentNullException("method");
			string url = m_baseUrl + "/" + method;
			var sw = new System.Diagnostics.Stopwatch();
			sw.Start();
			var eventData = new HttpResponseHandlerData(url, method, null, null, headers);
			if (OnStart != null)
			{
				OnStart(eventData);
			}
			...
			byte[] postData;
			using (var inStream = m_serializer.Serialize(requestData))
			{
				postData = inStream.GetBytes();
			}
			...
			{
				httpClient.Timeout = TimeSpan.FromSeconds(timeoutInSeconds.Value);
			}
			...
			req.Content = new ByteArrayContent(postData);
			...
			{
				req.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue(acceptType));
			}
			...
			var streamTask = httpClient.SendAsync(req).ContinueWith<Task<T>>((rspTVal) => {
				string respHeadersStr = "";
				HttpResponseMessage response = null;
				try
				{
					response = rspTVal.Result;

					var respHeaders = new NameValueCollection();
					if (response.Content != null && response.Content.Headers != null)
					{
						foreach (var h1 in response.Content.Headers)
						{
							respHeaders.Add(h1.Key, string.Join(";", h1.Value));
						}
						respHeadersStr = response.Content.Headers.ToString();
					}

					if (response.Headers != null)
					{
						foreach (var h2 in response.Content.Headers)
						{
							respHeaders.Add(h2.Key, string.Join(";", h2.Value));
						}
						respHeadersStr += response.Headers.ToString();
					}

					if(response.IsSuccessStatusCode)
					{
						return response.Content.ReadAsStreamAsync().ContinueWith<T>((strmTVal) => {
							try
							{
								using(var strm = strmTVal.Result)
								{
									if(OnComplete != null)
									{
										sw.Stop();
										eventData.Stage = EventKind.Complete;
										eventData.Response = strm.GetBytes();
										eventData.DurationMilliseconds = (int)sw.ElapsedMilliseconds;
										eventData.ResponseHeaders = respHeaders;
										eventData.Status = response.StatusCode.ToString();
										OnComplete(eventData);
									}
									if(hasResponse)
									{
										...
										{
											var respStatus = response == null ? HttpStatusCode.RequestTimeout : response.StatusCode;
											throw new BabelRequestException("Invalid response format", "RESPONSE_FORMAT_ERROR", respStatus, url, respHeaders == null ? "" : respHeadersStr, strm.GetString(), err);
										}
									}
									else
									{
										return default(T); //We use string for that, so it is null
									}
								}
							}
							finally
					...
					{
						if(retriesLeft > 0 && ConsiderRetry(response))
						{
							try
							{
								if(OnFailure != null)
								...
								var delayTask = Task.Delay(this.RetryDelay).ContinueWith(t => MakeHTTPRequestAsync<T>(httpMethod, method, requestData, contentType, acceptType, timeoutInSeconds, headers, hasResponse, retriesLeft - 1)).Unwrap();
								return delayTask.ContinueWith(t => t.Result);
							}
							finally
							{
								httpClient.Dispose();
							...
							return response.Content.ReadAsStreamAsync().ContinueWith<T>((strmTVal) => {
								try
								{
									using(var strm = strmTVal.Result)
									{
										...
										try
										{
											error = m_serializer.Deserialize<ServiceError>(strm);
										}
										catch
										{
											error = null; //This is not a service error
										}

										if(error == null)
										{
											error = new ServiceError {
												Details = string.Format("HTTP Error: {0} ({1})", response.ReasonPhrase, (int)response.StatusCode)
											};
										}

										var errorHeaders = error.GetContextCategory("HTTPHeaders");
										foreach(string k in respHeaders)
										{
											errorHeaders[k] = respHeaders[k];
										}

										var kind = GetErrorKind(response.StatusCode);
										var err = BabelException.FromServiceError(error, kind);
										if(OnFailure != null)
										{
											sw.Stop();
											eventData.Stage = EventKind.Failure;
											eventData.Response = respBytes;
											eventData.DurationMilliseconds = (int)sw.ElapsedMilliseconds;
											eventData.ResponseHeaders = respHeaders;
											eventData.Status = response.StatusCode.ToString();
											eventData.Error = err;
				...
				catch (Exception err)
				{
					...
					if (response != null) response.Dispose();
					HttpStatusCode statusCode = response != null ? response.StatusCode : HttpStatusCode.InternalServerError;
					throw new BabelRequestException("Error connecting to", "REQUEST_ERROR", statusCode, url, respHeadersStr, "", err);
				}
				finally
				{
					req.Dispose();
		...
		protected virtual Task<BabelWebResponse> MakeHTTPRequestAsync(string httpMethod, string url, byte[] postData, string contentType, string acceptType, int? timeoutInSeconds, NameValueCollection headers)
		{
			...
			if(headers != null)
			{
				foreach(string key in headers)
				{
					...
					//Don't including restricted headers - they will throw the error. One way would be to writre
					switch(lck)
					{
						case "content-length": 
						case "content-type": 
		...
		private static Task<BabelWebResponse> GetResponseWithTimeoutAsync(HttpWebRequest request, string url, int? timeoutInSeconds)
		{
			...
			if(timeoutInSeconds.HasValue && timeoutInSeconds.Value > 0)
			{
				...
				return timeoutTask.ContinueWith<Task<WebResponse>>((tt) => {
					...
					{
						request.Abort();
						//Now need to eat cancelled exception
						try
						{
							responseTask.Wait();
						}
		...
		private static BabelWebResponse GetResponseAsync(Task<WebResponse> timeoutWaitRes, string url)
		{
			...
			catch(AggregateException aerr)
			{
				...
				{
					HttpStatusCode statusCode = response != null ? response.StatusCode : HttpStatusCode.InternalServerError;
					return new BabelWebResponse(statusCode, url, err, outHeaders, Encoding.ASCII.GetBytes(err.Message)); // .. or should it be null instead of the error texp
				}
			}
			finally
			{
