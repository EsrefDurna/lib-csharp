...
namespace BabelRpc
{
	class DummySerializer : IBabelSerializer
	{
		#region IBabelSerializer Members
		public T Deserialize<T>(Stream str)
		{
			if (typeof(T) == typeof(ServiceError)) return (T)(object)new ServiceError();
			return default(T);
		}
	...
	public class RetryTests : HttpTransport
	{
		...
		public void ZeroRetriesStillTries(){
			...
			transport.m_serializer = new DummySerializer();
			int failues = 0;
			int completes = 0;
		...
		public void OneRetryAsync()
		{
			...
			Assert.AreEqual(0, completes, "Should have no completes");
		}
